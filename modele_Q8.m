function [uv,uv_th, tab_sigma, tab_sigma_th, Coord, Connect] = modele_Q8(nRadius,nTheta)
    ...
    nnel = 8;%8%9 en fonction
    % Nombre de noeud de l'élément de contour
    nnelc=3;
    % Calcul du nombre de DDL par élément interne (16 dans le cas du Q8)
    Nui=ddln*nnel;
    ...
    [matNoeud, matConnectelemint, ContourB, ~, DirichletY, DirichletX] = maillage_2D(params, dims);
    %visu_maillage('polaire',matNoeud,matConnectelemint)
    ...
    NewmatConnec=zeros(nelin,nnel);
    NewmatConnec(1:nelin,1:2:end)=matConnec(1:nelin,1:end/2);
    NewmatConnec(1:nelin,2:2:end)=matConnec(1:nelin,end/2+1:end);
    matConnec(1:nelin,:)=NewmatConnec;
    for i=1:nelin
        for j=1:size(matConnec,2)/2
            c=matConnec(i,j);
            d=matConnec(i,end-j+1);
            matConnec(i,end-j+1)=c;
            matConnec(i,j)=d;
        end
    end
    ...
    matConnec(1:nelin,:)=circshift(matConnec(1:nelin,:),1,2);
    ...
    %===============================================================
    % 10. RÉSOLUTION DU SYSTÈME EN DÉPLACEMENT (u et v en m partout)
    %===============================================================
    sol= KG\FG;
    % AFFICHAGE DES RÉSULTATS EN DÉPLACEMENT
    %=========================================================

    cn=matConnec(1:nelin,:);  % Connectivités des quadrilatères seulement
    ...
    %===============================================================
    nntc=(nRadius+1)*(nTheta+1);%Nombre de noeud totaux aux coins
    ...
    contraintes_n=zeros(3,nnt);
    %matConnec2=matConnec(1:nelin,1:2:end);
    clear coor nd
    for ie=1:nelin  % Pour tous les éléments internes
        ...
        for ig=1:npg
            ...
            me= me + W(ig)*(Nnum')*Nnum*DJ;  % matrice masse qui va servir pour le lissage
    ...
    sigmay= MG\GY;
    sigmaxy= MG\GXY;
    % sigmax = interpQ4_to_Q8(sigmaxQ4,params,dims);
    % sigmay = interpQ4_to_Q8(sigmayQ4,params,dims);
    % sigmaxy = interpQ4_to_Q8(sigmaxyQ4,params,dims);
    ...
    sigmat=zeros(nnt,1);
    sigmart=zeros(nnt,1);
    sigmarth=zeros(nnt,1);
    sigmatth=zeros(nnt,1);
    sigmartth=zeros(nnt,1);
    sigmaxth=zeros(nnt,1);
    sigmayth=zeros(nnt,1);
    sigmaxyth=zeros(nnt,1);
    for i=1:nnt
        x=matNoeud(i,1);
        y=matNoeud(i,2);
        theta=atan(y/x);
        ...
        sigmaxyth(i)=cos(theta)*sin(theta)*(sigmarth(i)-sigmatth(i))+sigmartth(i)*cos(2*theta);
